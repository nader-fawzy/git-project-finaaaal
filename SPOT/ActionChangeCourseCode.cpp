...
bool ActionChangeCourseCode::Execute() {
	...
	if (actData.actType == DRAW_AREA) {  //get coord where user clicked...
		...
		Course* PCourse = pS->DetectCourse(x_point, y_point, semesters.year, semesters.s); //detect course position
		//int* point = pS->DeleteCourse(x_point, y_point); //delete the selected course
